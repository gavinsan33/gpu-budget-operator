...
func periodStart(period gpubudgetv1alpha1.BudgetPeriod, now time.Time) time.Time {
	now = now.UTC()
	switch period {
	case gpubudgetv1alpha1.PeriodDaily:
		return time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, time.UTC)
	case gpubudgetv1alpha1.PeriodWeekly:
		day := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, time.UTC)
		// time.Weekday: Sunday=0 ... Saturday=6. Days since the most recent Monday.
		daysSinceMonday := (int(day.Weekday()) + 6) % 7
		return day.AddDate(0, 0, -daysSinceMonday)
	case gpubudgetv1alpha1.PeriodMonthly:
