...
type GpuQuotaSpec struct {
	...
	// operator's configured per-GPU-family hourly rates (one or more
	// --gpu-rate=<family>=<usd> flags, e.g. --gpu-rate=A100=1.70). At least
	// one of GPUHoursLimit/DollarsLimit must be set; if both are set,
	// whichever is exceeded first triggers enforcement.
	// +optional
